...
function PokemonCacheProvider({children}) {
  const [pokemonCache, dispatch] = React.useReducer(pokemonCacheReducer, [])

  return (
    <PokemonCacheContext.Provider value={[pokemonCache, dispatch]}>
      {children}
...
function usePokemonCache() {
  ...
  if (!context) {
    throw Error('Use usePokemonCache inside of a PokemonCacheProvider')
  }
